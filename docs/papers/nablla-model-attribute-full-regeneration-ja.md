# Sercrod 型モデル: 属性駆動・全再構築による非 Virtual DOM Web UI アプローチ
...
という一方向の変換過程を中核とした抽象的な設計原理として定義される。その具体的な運用例として、Sercrod（仮称）を参照しながら記述する。

近年の Web 開発では、仮想 DOM 型や宣言的 UI など、DOM 更新を抽象化する多様な方式が提案されてきた。これらはしばしば、内部に仮想的な構造を保持し、差分を計算して実 DOM へ反映する。

これに対し Sercrod は、仮想構造を持たず、HTML の属性と DOM をそのまま UI 記述の主役とみなし、UI の変化を
...
Sercrod は Custom Elements（Web Components）を利用するが、速度向上や局所描画を Web Components の「隠れた最適化」に求めるものではない。本稿での観測結果は、

- ブラウザや DOM 実行環境は、JavaScript による DOM 操作に忠実に従って layout / paint を実行する  
- Web Components であること自体が特別な差分描画を提供するわけではない  

という事実を示している。  
Sercrod の特徴は、Web Components の内部に「属性を正とする UI モデル」を閉じ込め、ブラウザ標準の DOM エンジンに直接乗せるという構造にある。

本稿の目的は、Sercrod を個別のプロダクトとして宣伝することではなく、そこで得られた設計上・観測上の結果を、
...
- Sercrod の内部構造と処理手続きの抽象化  
- 仮想 DOM 型モデルとは異なる UI モデルとしての位置づけ  
- 属性中心モデルの学術的・実務的意義の整理  

以降、Sercrod という名称は、特定の実装を指す固有名詞であると同時に、「属性駆動・全再構築モデル」を具現化した一例として用いる。
...
という層構造を採用するのに対し、Sercrod 型モデルは、

> 状態（scope） → 属性 → DOM

という、より直接的な三段階を前提とする。  
ここで重要なのは、Sercrod 自身が「仮想 DOM を持たない」という点であり、「仮想 DOM の性能を超える」といった比較ではない。
...
といったポイントが挙げられる。  
Sercrod 型モデルは、この層構造そのものを採用せず、「属性を正とする」思想のもと、DOM を直接状態の唯一の写像先として扱う。
...
1. **スコープの境界**  
   各 Sercrod 要素はひとつの Custom Element であり、データ scope・テンプレート・ライフサイクルを局所的に持つ。
...
以上を踏まえると、Sercrod 型のモデルは概念的に次のように位置づけられる。
...
本章では、「属性を正とする」Sercrod モデルの基本理念を整理し、なぜ attribute-first が必然的な選択となり得るのかを説明する。
...
Sercrod の attribute-first は、この立場を明示的な原則として採用している。
...
Sercrod では、各 Custom Element がテンプレート（template）を内部に保持する。しかし、このテンプレートは「別の世界の状態」ではなく、
...
Sercrod 型モデルでは、DOM とそこに付与された属性が「唯一の真実の所在」である。  
...
本章では、Sercrod 型モデルを可能にする Web 標準技術を整理する。
...
といった性質が得られる。  
Sercrod はこの性質を前提に、「属性＝状態」としての意味付けを行う。
...
Sercrod 型モデルは、追加のネイティブ API を要求せず、この標準技術の上に
...
本章では、「Web Components なら局所描画になる」「特別な最適化が働く」といった一般的な理解が、本稿の観測結果と一致しなかったことを示し、Sercrod 型モデルがどのような前提に立つべきかを整理する。
...
3. **大規模 DOM でも処理は現実的な時間で完了し得る**  
   ...
   これは DOM エンジンそのものの性能によるものであり、Sercrod が特別な描画制御を行った結果ではない。
...
- Sercrod の UI 更新は、ブラウザ標準の DOM 処理の範囲で完結している  
- 局所的に見える描画は、標準的な最適化の結果であり、Sercrod 独自の描画アルゴリズムではない  

### 5.4 Sercrod が採用する前提

この観測を前提に、Sercrod 型モデルは次のように立脚点を定める。
...
Sercrod の更新モデルは、仮想 DOM や複雑な差分計算を持たず、full regeneration という単純な構造を採用することによって、この前提と整合している。

---

## 第6章 Sercrod の独自性と属性駆動制御モデル

本章では、Sercrod 型モデルの独自性を、「描画最適化」ではなく「記述モデルと制御構造」に求めて整理する。

### 6.1 属性を正とする UI 記述体系

Sercrod は、UI の論理を HTML 属性として記述するための体系を提供する。
...
Sercrod の `update()` は、既存 DOM との比較を行わない。代わりに、
...
Custom Elements により、各 Sercrod 要素は独立したスコープとライフサイクルを持つ。  
これにより、
...
本章では、Sercrod の `update` 処理と内部モデルを、実装に依存しない抽象手続きとして整理する。
...
- 子コンポーネント（入れ子の Sercrod 要素）があれば、それぞれに `update()` を呼び出す  
- 属性・テキストノードを反映する  
...
この処理過程により、Sercrod 型モデルは次の性質を持つ。
...
本章では、前章までの構造に基づき、Sercrod 型モデルがもたらす利点を整理する。
...
Sercrod 型モデルは、DOM エンジンを直接利用する。仮想 DOM や独自の差分層を挟まないため、
...
Sercrod のコードは、HTML ドキュメントの中に完結して存在する。  
これにより、
...
本章では、Sercrod 型モデルを概念モデルとして捉えたときの位置づけと、今後の応用可能性を整理する。

### 9.1 概念モデルとしての三本柱

Sercrod 型モデルは概念的に、次の三つの柱から整理できる。
...
## 第10章 まとめ ― Sercrod 型モデルの体系

本稿では、Sercrod（仮称）を参照しながら、
...
という構造を持つ。  
描画の最適化は DOM エンジンに委ねられ、Sercrod 自身は「どのように DOM と属性を扱うか」という設計原理に注力する。
...
本付録では、Sercrod 型モデルの `update` 手続きの流れを段階的に定義する。
...
- 子コンポーネント（入れ子の Sercrod 要素）の `update` 呼び出し  
- 属性・テキストノードの反映  
...
本付録では、Sercrod 型モデルの内部構造を形式的に定義する。
...
本内部モデルは、Sercrod 型モデルが採用する設計上の前提と保証を明示し、属性駆動・全再構築という UI モデルを形式的に説明するための基盤である。
